...
import com.intarea.intarea.dto.PredictAResponse;
import com.intarea.intarea.dto.PredictAResult;
import com.intarea.intarea.repository.PredictionAOrderRecordRepository;
...
@AllArgsConstructor
@Slf4j
public class PredictAService {

    private final PredictionAOrderRecordRepository predictionRecordRepository;

    // 예측을 위해 요청을 보낼 Flask API 엔드포인트 주소값을 상수 지정.
    private final String FLASK_URL = "http://localhost:8000/predictA";

    // 예측 요구값을 받아 예측 결과를 반환하는 메서드
    public PredictAResult getPrediction(PredictARequest request) {
        ...
        HttpEntity<PredictARequest> entity = new HttpEntity<>(request, headers);

        // 1-4. Flask API 호출 - exchange()로 POST요청 보내기.
        ResponseEntity<PredictAResponse> response = restTemplate.exchange(
                FLASK_URL,
                HttpMethod.POST,
                entity,
                PredictAResponse.class // 결과값을 PredictResponse.class형태로 자동 변환.
        );
        ...
        PredictionAOrderInput orderInput = request.getOrders().get(0);

        PredictionAOrderRecord record = new PredictionAOrderRecord();
        ...
        // 6. 결과값 받아 반환
        return new PredictAResult(prediction);
